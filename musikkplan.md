# Musikkplan – mellom talerne

Strategisk plan for hvordan låtene i «Spilleliste 80» kan brukes som intro,
outro og energijustering mellom talere. Låtene er valgt ut fra
bryllupsbriefen – humorvinkler, talerprofiler og rødetrådsmetaforer.

**Prinsipp**: kort snutt (20–40 sek) ved intro/outro er nok. Full låt
reserveres til pauser eller dansegulv.

---

## 1. Oversikt – låtene sortert etter funksjon

### 🔊 Fanfarer / korte energiløft (intro før taler)

Korte, umiddelbare – perfekte for å bygge forventning akkurat før en taler
reiser seg.

| # | Låt | Artist | Lengde | Best til |
|---|-----|--------|--------|----------|
| 6 | Let's Get Ready To Rumble! | Zantro | 1:59 | Forlover (brudgom) – punchy intro |
| 3 | Macgyver Theme | Hollywood TV Players | 1:14 | Erik Nilsen – «problemløseren» |
| 36 | Free Bird (MOONLGHT) | MOONLGHT | 1:54 | Ola – biler / frihet |
| 21 | Stayin' Alive | Bee Gees | 1:33 | Generell løfter |

### 🎤 Talerkoblinger – myke / varme

| # | Låt | Passer til | Grunn |
|---|-----|-----------|-------|
| 7 | Fanga av ein stormvind | **Forlover (brud)** | Nordnorsk, poetisk, natur |
| 29 | The Power Of Love | **Brudens far** | Farsvisdom, tyngde |
| 12 | Walking On Sunshine | **Cathrine + Anette (felles tale, søstre)** | Varmt, oppveksthistorier |
| 32/33 | Crazy Train | **Ingrid + Ola (felles tale, Anders' søsken)** | Bil/sport-tema |
| 9 | Sarà perché ti amo | Overgang / romantisk | Italiensk sjarm |
| 28 | Take on Me | Intro som løfter blidt | Klassiker, 80-tall |

### ⚡ Talerkoblinger – høyt tempo / humor

| # | Låt | Passer til | Grunn |
|---|-----|-----------|-------|
| 20/35 | Thunderstruck | **Forlover (brudgom)** | Gym, prestasjon |
| 13 | Enter Sandman | **Forlover (brudgom)** alt. | Tung intro |
| 32/33 | Crazy Train | **Ola** eller **Anders** | Bilmetafor |
| 23 | T.N.T. | **Ola** | Sport / krefter |
| 24 | Hold the Line | **Erik Nilsen** | «Hold linjen» – logistikk |
| 30 | We Didn't Start the Fire | **Erik Nilsen** alt. | Driv, tog-tempo |

### 🚀 Brudgom / Equinor / start-up

| # | Låt | Bruk |
|---|-----|------|
| 13 | Enter Sandman | Når Anders reiser seg – «pitch deck mode» |
| 20 | Thunderstruck | Anders' takketale – kraft |
| 32 | Crazy Train | Humor-intro: «Bli med på turen» |

### 👰 Brud / baking / DIY

| # | Låt | Bruk |
|---|-----|------|
| 19 | Mamma Mia | Når bruden reiser seg (hvis hun taler) |
| 22 | Voulez-Vous | Elegant overgang |
| 16 | Waterloo | Klassisk bryllupsløfter |
| 26 | Burning Love | Anders' tale til bruden – outro |
| 11 | Signed, Sealed, Delivered | Etter ringene / takketale |

### 🇳🇴 Lokale / norske perler (spar til høydepunkter)

| # | Låt | Bruk |
|---|-----|------|
| 1 | Forza Vinger – Blodrødt | Åpning hvis lokalt publikum kjenner den |
| 2 | Shock 2017 (kongsvingerrussen) | Sent, hvis russeprofil passer |
| 14 | Optimist – Jahn Teigen | Før Erik / før brudens far – norsk tyngde |
| 31 | Bønda i fra nord 2000 | Sent – gir fartsfylt stemning |
| 8 | Vi Kommer Fra Skogene | Etter middag – inn i festdelen |

### 🎉 Festtoppere (etter talene / åpning av dansegulv)

203 BPM-låtene (Vi Kommer Fra Skogene, Country Roads HHB) er **for raske for
middagsbakgrunn** – hold disse til dansen.

| # | Låt | Bruk |
|---|-----|------|
| 4 | Samba de Janeiro | Når kaken kuttes / brudepar reiser seg |
| 5 | Cotton Eye Joe | Åpning av dansegulv |
| 15 | Country Roads (HHB) | Fellessang-øyeblikk |
| 8 | Vi Kommer Fra Skogene | Full fest |
| 10 | Y.M.C.A. | Garantert gulvfyller |
| 25 | September | Midt på natten |
| 27 | Boogie Wonderland | Midt på natten |

---

## 2. Forslag – intro-par per taler

Én forslått låt + én reserve. Kort snutt, fade ut når taleren står ved mikrofonen.

| Taler | Primær | Reserve | Lengde på intro |
|-------|--------|---------|-----------------|
| **Tale 1 – Werner Seigerud** (brudens far, FØR mat) | Optimist – Jahn Teigen (#14) | The Power Of Love (#29) | 20–25 sek |
| **Tale 2 – Anders til Michelle** (FØR mat) | Burning Love (#26) | The Power Of Love (#29) | 20 sek |
| **Tale 3 – Michelle til Anders** (brudens tale, FØR mat) | Mamma Mia (#19) | Burning Love (#26) | 20 sek |
| **Tale 4 – Kjersti & Erik Nilsen** (Anders' foreldre, forrett) | Power of Love (#29) | Signed, Sealed, Delivered (#11) | 20 sek |
| **Tale 5 – Helene** (forlover brud) | Fanga av ein stormvind (#7) | Take on Me (#28) | 20–25 sek |
| **Tale 6 – Even** (forlover brudgom) | Let's Get Ready To Rumble! (#6) | Thunderstruck (#20/35) | 10–15 sek |
| **Tale 7 – Cathrine + Anette** (felles, Michelles søstre) | Walking On Sunshine (#12) | Mamma Mia (#19) | 20 sek |
| **Tale 8 – Ingrid + Ola** (felles, Anders' søsken) | Crazy Train (#32) | T.N.T. (#23) | 15 sek |
| **Tale 9 – Kongsvinger-vennene** (gruppe-tale) | Forza Vinger – Blodrødt (#1) | Shock 2017 (#2) | 20 sek |
| **Tale 10 – «Feminin og fornem»** (Ås-gjeng Michelle) | Voulez-Vous (#22) | Sarà perché ti amo (#9) | 20 sek |
| **Innslag – Broderskapet Unity** (Anders' Ås-vennegjeng) | Enter Sandman (#13) | T.N.T. (#23) | 15 sek |
| **Tale 11 – Tom Christian** (kveldens siste) | Hold the Line (#24) | We Didn't Start the Fire (#30) | 15 sek |

---

## 3. Dramaturgisk tempo-kurve gjennom middagen

Unngå å følge rask låt med rask låt. Behold Duartes prinsipp om
`hva er → hva kan bli` – men musikalsk.

```
Åpning           ██        Optimist / Forza Vinger – varm start
Werner Seigerud  ██        Power Of Love – senke skuldre
Anders til brud  ███       Burning Love – løft
Michelle         ███       Mamma Mia – feminin og fornem (paret-utveksling)
Helan går        ██        Skål
Fireball 1       ███       Velkomstshot
— buffet 1 (forrett) —
Kjersti+Erik N.  ██        Power Of Love – arv eller miljø
— buffet 2 (hovedrett) —
Helene (f. brud) ██        Fanga av ein stormvind – ro
Even  (f. brudg) ████      Let's Get Ready / Thunderstruck – topp
Cathrine+Anette  ██        Walking On Sunshine – søster-duo
Ingrid+Ola       ████      Crazy Train – søsken-duo
— pause + Fireball 2 —
— dessert —
Kongsvinger      ██        Forza Vinger – lokal forankring
Feminin og fornem██        Voulez-Vous – Ås-gjeng Michelle
Broderskapet     ████      Enter Sandman – innslag (Anders' Ås)
Tom Christian    ███       Hold the Line – KVELDENS SISTE TALE
Stikk 4          ███       Take on Me + Fireball 3
— kake / dans —
                 █████     Samba / Cotton Eye Joe / Y.M.C.A.
```

Maks to «firere» på rad. Brekk alltid med en «toer» eller pause.

---

## 4. DJ-teknisk (Camelot-nøkler & BPM)

«Spilleliste 80» har Camelot-koder i Toneart-kolonnen. For glatte overganger
uten kollisjon:

- **Harmonisk mix**: samme tall (f.eks. 10B → 10B), eller ±1 (10B → 11B),
  eller samme tall med bokstavskifte (10B → 10A).
- **Risikofylt hopp**: Enter Sandman (12B 123 BPM) rett til Take on Me
  (11B 84 BPM) – stor tempodropp.
- **Trygge par**:
  - Waterloo (10B 148) → Mamma Mia (10B 138) → Burning Love (10B 144)
  - Thunderstruck (1B 133) → September (1B 126)
  - Crazy Train Ozzy (11B 138) → Cotton Eye Joe (11B 132) → Take on Me (11B 84)

---

## 5. Fallgruver

- **Kongsvingerrussen (#2)**: sjekk at gjester under 20 og over 60 takler
  den – ellers vent til sen kveld.
- **Vi Kommer Fra Skogene (#8) 203 BPM**: ikke bruk som taler-intro, den
  stresser rommet.
- **Enter Sandman**-intro (#13) kan virke truende for middagsgjester som
  ikke er AC/DC/Metallica-fans. Bruk kort snutt (10 sek) eller hold den
  til Anders' egen takketale.
- **To Thunderstruck-versjoner (#20 og #35)**: velg én – ikke spill begge.
  MOONLGHT-versjonen (2:32) er kortere og renere for intro.
- **Tre MOONLGHT-spor på rad (#33/#35/#36)**: samme produksjonsstil, kan
  virke monotont – brekk med en klassiker.

---

## 6. Nødlåter (hvis tid må fylles)

Hvis en tale blir avlyst eller servering forsinkes:

1. Samba de Janeiro (#4) – instant løft.
2. Y.M.C.A. (#10) – fellesaksjon med armene.
3. Country Roads HHB (#15) – fellessang.
4. Walking On Sunshine (#12) – universelt smil.

---

## 7. Avklar med Simen (lydtekniker)

**Simen (#57, brudesvenn) er lydtekniker og styrer all musikk.** Hans
fulle brief med pakkeliste, cue-liste og frister ligger i `simen-lyd.md`.

- [x] Hvem styrer avspilling under middagen – **Simen**.
- [ ] Signaler for fade-in/fade-out (håndtegn / headset) – avtales på generalprøven 25. juni.
- [ ] Preview-mulighet: test at alle 36 spor er tilgjengelige i systemet (offline!).
- [ ] Bekreft volum under middag (snakkevennlig) vs. dans (høyt).
- [ ] Avtal «panic track» (Samba de Janeiro) som Simen kan dra i hvis du signaliser.
- [ ] Simen får full musikkliste + cues **senest 19. juni** (7 dager før).
